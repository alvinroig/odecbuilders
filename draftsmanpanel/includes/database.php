...
function fetchProjectReport($id){
            ...
            try{
                ...
                while($row = $query->fetch()){
                    ...
                    echo '<td><a href="viewLogs.php?id='.$row['pid'].'"><button class="btn btn-info btn-sm" style="width:100%;" type="submit"><span class="glyphicon glyphicon-stats"></span> View Logs</button></a></td>';
                    echo '</tr>';
...
function fetchprojm($id){
    try{
                ...
                while($row = $query -> fetch() ) {
                    ...
                    echo "<input type='text' value='".$row["pn"]."' class='form-control' readonly />";
                }
            }catch(PDOException $ex){
